...
import { FakeBikeRepo } from "./fake-bike-repo"
import { FakeRentRepo } from "./fake-rent-repo"
import { UserRepo } from "../../src/ports/user-repo"
import { BikeRepo } from "../../src/ports/bike-repo"
import { RentRepo } from "../../src/ports/rent-repo"
import { RentNotFound } from "../../src/errors/rent-not-found-error"
import { userOpenRentError } from "../../src/errors/user-open-rent-error"
...
describe('App', () => {
    ...
    it ('should when rent not found', async () => {
        const app = new App(userRepo, bikeRepo, rentRepo)
        const user = new User('jose', 'jose@mail.com', '1234')
        const bike = new Bike('caloi mountainbike', 'mountain bike',
            1234, 1234, 100.0, 'My bike', 5, [])
        await app.registerUser(user)
        await app.registerBike(bike)
        await expect(app.returnBike(bike.id, user.email)).rejects.toThrow(RentNotFound)
    })

    it('should when the user to be removed had open rent', async() => {
        const app = new App(userRepo, bikeRepo, rentRepo)
        const user = new User('jose', 'jose@mail.com', '1234')
        const bike = new Bike('caloi mountainbike', 'mountain bike',
            1234, 1234, 100.0, 'My bike', 5, [])
        ...
        await app.registerBike(bike)
        await app.rentBike(bike.id, user.email)
        await expect(app.removeUser(user.email)).rejects.toThrow(userOpenRentError)
    })
})